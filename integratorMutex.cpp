...
//TODO: Implement spin buffers for data storage, seems like a good idea
...
char *IntegratorNames[NUMINTEGRATORS]=
	{
         "Map",
	 "Euler",
	 "Gear1",
...
char *FailLock="Failed to lock memory in Integrator thread";
//-----------------------------------------------------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------------------------------------------------
Integrator::Integrator(QMutex *p) : QThread()
{
 Q_ASSERT(p!=NULL);
 nIntegrat=RK4;
 eps=2.0e-8;
...
//---------------------------------------------------------------------------
void Integrator::RegisterEquation(NS_Equation::Equation *Equation)
{
 if (Equation!=NULL)
    {
...
{
 //see if we can get the mutex object:
 if (!pMutex->tryLock())
 {
  bLocked=false;
...
bool Integrator::LockMutex()
{
 ...
 {//could not lock at this point,try for LOCK_TRY times for INT_LOCK_MS (=n*m s) to lock it
   int i=0;
...
void  Integrator::doLyapStep()
{
 ...
 if ((uStep%nLyapMod)==0)
	...
	//calc running average
 	transform(dvLyapSum.begin(),dvLyapSum.end(),dvLyapAvg.begin(),bind2nd(divides<double>(),vddLyap[0].size()));
...
void  Integrator::Gear1()
{
 ...
 try 
    {
      NS_Formu::DblVector yt(N);
      NS_Formu::DblVector dyt(N);
...
void Integrator::rk2()
{
  ...
  try
    {
      NS_Formu::DblVector ak1(N);
      NS_Formu::DblVector ak2(N);
      NS_Formu::DblVector ak3(N);
      NS_Formu::DblVector ytemp(N);
  ...
  try
    {
      NS_Formu::DblVector ak1(N);
      NS_Formu::DblVector ak2(N);
      NS_Formu::DblVector ytemp1(N);
      NS_Formu::DblVector ytemp2(N);
...
void Integrator::rk45()
{
  ...
  NS_Formu::DblVector ak2(N);  
  NS_Formu::DblVector ak3(N);  
  NS_Formu::DblVector ak4(N);  
  NS_Formu::DblVector ak5(N);  
  NS_Formu::DblVector ak6(N);  
...
void  Integrator::rkpd()
{
  ...
  NS_Formu::DblVector ak5(N);
  NS_Formu::DblVector ak6(N);
  NS_Formu::DblVector ak7(N);
  NS_Formu::DblVector ak8(N);
  NS_Formu::DblVector ak9(N);
  NS_Formu::DblVector ak10(N);
  NS_Formu::DblVector ak11(N);
  NS_Formu::DblVector ak12(N);
  NS_Formu::DblVector ak13(N);
...
void  Integrator::rkck(NS_Formu::DblVector &yout,NS_Formu::DblVector &yerr,double hl)
{
 unsigned int i;
